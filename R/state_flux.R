data <- read.csv(here::here("data/fia_nys_report/FRF_net_flux_by_State.csv")) |>
  ...
  dplyr::filter(Carbon.Pools == 'Aboveground Biomass') |>
...
state_flux <- ggplot(data |> dplyr::filter(Net.Flux.Unit == 'MMT C')) +
  ...
  scale_y_continuous(expand = c(0.005, 0.005),
                     limits = c(-6, -4)) +
  ylab(bquote('Million Mg'~year^-1*'')) +
  xlab('Year') +
  ggtitle("Annual Aboveground Carbon Flux - NYS") +
  theme_minimal() +
  theme(
